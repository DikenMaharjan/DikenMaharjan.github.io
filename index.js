...
var gameState = {};
var chooseTimers = {};
var chooseWordRemainingTime = {};
var gameTimeRemainingTime = {};
var gameTimers = {};
var scoreTimers = {};
...
function startChooseWordTimer(id) {
  chooseWordRemainingTime[socketPlusRooms[id]] = 15;
  chooseTimers[socketPlusRooms[id]] = setInterval(() => {
    chooseWordRemainingTime[socketPlusRooms[id]] -= 1;
    io.to(id).emit("chooseWordTimer", chooseWordRemainingTime[socketPlusRooms[id]]);
    if (chooseWordRemainingTime[socketPlusRooms[id]] <= 0) {
      clearInterval(chooseTimers[socketPlusRooms[id]]);
...
function startGameTimer(room) {
  ...
  gameTimeRemainingTime[room] = time[gameInformations[room].time];
  gameTimers[room] = setInterval(() => {
    remainingTime = gameTimeRemainingTime[room];
    remainingTime -= 1;
    gameTimeRemainingTime[room] = remainingTime;
...
const chooseWord = (room) => {
  io.to(gameState[room].drawerId).emit("chooseWordTimer", 15);
...
io.on("connection", (socket) => {
  ...
  socket.on("wordChosen", (word) => {
    clearInterval(chooseTimers[socketPlusRooms[socket.id]]);
    io.to(socketPlusRooms[socket.id]).
      emit("gameStarted", socket.id);
    io.to(socketPlusRooms[socket.id]).emit("gameTimer", time[gameInformations[socketPlusRooms[socket.id]].time]);
  ...
  socket.on("disconnect", (reason) => {
    room = socketPlusRooms[socket.id];

    if (room) {
      delete allPlayers[room][socket.id];
      playerTurns[room].splice(playerTurns[socketPlusRooms[socket.id]].indexOf(socket.id), 1);
      numOfReady[room] -= 1;
      numberOfPlayers[room] -= 1;
      if (numberOfPlayers[room] == 0) {
        clearTimeout(scoreTimers[room]);
        delete scoreTimers[room];
        clearInterval(gameTimers[room]);
        delete gameTimers[room];
        clearInterval(chooseTimers[room]);
        delete chooseTimers[room];
        delete allPlayers[room];
        delete gameInformations[room];
        delete playerTurns[room];
        delete numberOfPlayers[room];
        delete numOfReady[room];
        delete gameState[room];
        delete gameTimeRemainingTime[room];
        delete chooseWordRemainingTime[room];
      }
